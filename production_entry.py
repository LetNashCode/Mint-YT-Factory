...
def _patch_scene7_sanitizer(main):
    ...
    def preserve_bridge(scene7,earlier_scenes):
        ...
        for i,sentence in enumerate(sentences):
            if i==len(sentences)-1: kept.append(sentence); continue
...
def _patch_scene7_validator(main):
    ...
    def validate(scene7_narration,next_topic):
        ...
        if not text or not key: raise RuntimeError("Scene 7 must contain a payoff followed by a natural continuation bridge.")
        normalized=re.sub(r"[^a-z0-9]+"," ",text.lower()).strip()
        if normalized.count(key)!=1: raise RuntimeError("The exact next topic must appear exactly once in Scene 7.")
        sentences=[s.strip() for s in re.split(r"(?<=[.!?])\s+",text) if s.strip()]
        matches=[s for s in sentences if key in re.sub(r"[^a-z0-9]+"," ",s.lower()).strip()]
        if len(sentences)==1:
            if normalized.endswith(key): return text
            raise RuntimeError("Scene 7 must end with the continuation topic.")
        if not matches or matches[-1]!=sentences[-1]: raise RuntimeError("The continuation topic must be in Scene 7's final sentence.")
        bridge=matches[-1]
        ...
        if any(re.search(p,bridge,re.I) for p in banned): raise RuntimeError(f"Canned continuation bridge rejected: {bridge}")
        word_count=len(re.findall(r"\b[\w'-]+\b",bridge))
        if word_count<4 or word_count>40: raise RuntimeError("Natural continuation bridge is too short or too long.")
        return bridge
...
def _patch_locked_continuation(main):
    ...
    def lock(script,current_topic):
        ...
        if scenes:
            final=scenes[-1]; text=" ".join(str(final.get("narration","") or "").split()).strip(); next_topic=str((script.get("next_short") or {}).get("topic","") or "").strip(); key=re.sub(r"[^a-z0-9]+"," ",next_topic.lower()).strip(); normalized=re.sub(r"[^a-z0-9]+"," ",text.lower()).strip()
            if key and normalized.endswith(key) and len(re.findall(r"[.!?]",text))==0:
                ...
                if literal_pos>0:
                    ...
                    if prefix and prefix[-1] not in ".!?":
                        text=prefix+". "+text[literal_pos:].lstrip(); final["narration"]=text; final["subtitle_text"]=text
        return original(script,current_topic)
    lock._mint_bridge_normalizer=True; main.lock_next_topic=lock
...
def _patch_tts_duration(main):
    ...
    def synthesize(script,config,out_dir):
        ...
        for attempt in range(MAX_SHORT_TTS_REGEN+1):
            ...
            if duration>MAX_NARRATION_SECONDS: direction=f"The previous narration rendered at {duration:.2f} seconds and is TOO LONG. Rewrite it shorter. Target 90-100 words before the locked continuation. Remove filler, repeated explanations and extra setup while keeping the hook, escalation and payoff."
            else: direction=f"The previous narration rendered at {duration:.2f} seconds and is TOO SHORT. Target 100-110 words before the locked continuation. Add concrete everyday details and escalation, not scientific filler."
