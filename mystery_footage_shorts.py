...
def validate_case(item):
    ...
    if missing: raise RuntimeError(f"Catalog case {item.get('id', '<unknown>')} is missing required fields: {', '.join(missing)}")
    if "REPLACE_ME" in json.dumps(item): raise RuntimeError(f"Catalog case {item['id']} still contains a REPLACE_ME placeholder.")
    if not isinstance(item.get("verified_facts"), list) or not item["verified_facts"]: raise RuntimeError(f"Catalog case {item['id']} must contain verified facts.")
    if not isinstance(item.get("theories_or_open_questions"), list): raise RuntimeError(f"Catalog case {item['id']} must contain theories_or_open_questions.")
    if not item.get("rights_verified") and os.getenv("MYSTERY_FOOTAGE_ALLOW_UNVERIFIED", "false").lower() not in TRUE_VALUES: raise RuntimeError(f"Catalog case {item['id']} is not rights-verified; refusing to publish it.")
...
def load_item():
    ...
    for item in data.get("items", []):
        if not item.get("video_url") or not item.get("source_url") or "REPLACE_ME" in json.dumps(item): continue
        if not allow and item.get("rights_verified") is not True: continue
        validate_case(item); eligible.append(item)
    requested = os.getenv("MYSTERY_FOOTAGE_ITEM_ID", "").strip()
    if requested:
        for item in eligible:
            if item.get("id") == requested: return item
        raise RuntimeError(f"No eligible curated case matches MYSTERY_FOOTAGE_ITEM_ID={requested!r}.")
    if eligible: return eligible[0]
    raise RuntimeError("No eligible curated found-footage case is available. Add a case with verified facts, a matching footage URL, and rights metadata to mystery_footage_catalog.json.")
...
def inspect_video_frames(source, work):
    directory = work / "analysis-frames"; directory.mkdir(parents=True, exist_ok=True)
...
def generate_script(item, source, work):
    ...
    if not frames: raise RuntimeError("Could not extract frames for story grounding.")
    ...
    for field in ("video_title", "narration", "description_intro", "tags"):
        if not data.get(field): raise RuntimeError(f"Generated script is missing {field!r}.")
    return data

def download(item, destination):
    response = requests.get(item["video_url"], timeout=120, stream=True); response.raise_for_status()
    with destination.open("wb") as handle:
        for chunk in response.iter_content(chunk_size=1024 * 1024):
            if chunk: handle.write(chunk)
    if destination.stat().st_size < 10000: raise RuntimeError("Downloaded footage is unexpectedly small.")
...
def main():
    item = load_item(); OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="mystery-footage-") as temp:
        work = Path(temp); source = work / "source.mp4"; download(item, source)
        script = generate_script(item, source, work); audio = work / "narration.mp3"
        ...
        output = OUTPUT_DIR / "mystery-footage-short.mp4"; render(source, audio, output, work)
