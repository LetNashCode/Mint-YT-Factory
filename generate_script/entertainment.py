...
RESEARCH IS OFF. Do not cite sources or write like a textbook. Use ordinary knowledge
only when needed for coherence. Never invent precise statistics, studies, quotes or fake evidence.

VOICE:
Conversational, playful, curious, slightly quirky and confident. Short punchy sentences.
Sound like a clever human storyteller. Use vivid everyday comparisons. Explain technical ideas
through what a person can see, touch or imagine in ordinary life.
...
concept art, text, labels, logos, UI or watermarks unless the narration explicitly requires them.

TWO SHOTS PER SCENE:
Shot 1 establishes the exact moment. Shot 2 advances it by changing physical state,
action, viewpoint, comparison, reaction or revealed detail. Never duplicate Shot 1.

EVERY VISUAL MUST RETURN:
spoken_line: exact short narration beat represented by this shot.
visual_focus: main visible subject.
visual_action: physical action/state visible in the frame.
must_show: 3–6 concrete visible items/features.
must_not_show: 3–8 likely wrong/unrelated elements.
image_prompt: 25–45 words, literal, concrete, realistic and physically plausible.

REALISM:
Prefer cinematic photography or macro photography. Use realistic 3D only when necessary.
Natural materials, believable scale, realistic reflections, shadows and physics.
No illustration look.

MOTION:
Hooks can use strong motion. Explanations use controlled push-ins. Demonstrations use
pans that follow the action. Payoffs use a punchy reveal. Do not make every shot a zoom.
...
def _build_schema():
    visual = {
        ...
        "properties": {
            ...
            "spoken_line": {"type": "string"}, "visual_focus": {"type": "string"},
            "visual_action": {"type": "string"}, "must_show": {"type": "array", "items": {"type": "string"}},
        ...
        "required": ["segment", "duration", "spoken_line", "visual_focus", "visual_action", "must_show", "must_not_show", "camera", "animation", "zoom_strength", "motion_intensity", "visual_complexity", "image_style", "lighting", "color_palette", "overlay", "image_prompt", "visual_impact"],
    }
    scene = {
        "type": "object",
        "properties": {
...
def _normalize(script, topic):
    ...
    script["thumbnail_prompt"] = _clean(script.get("thumbnail_prompt"))[:500]
    ...
    script["visual_identity"] = {
        "style": _clean(identity.get("style")) or "cinematic realistic storytelling, tactile detail, believable real-world photography",
        "palette": _clean(identity.get("palette")) or "natural colors, crisp highlights, believable contrast",
    ...
    for i, scene in enumerate(scenes):
        ...
        scene["source_ids"] = []
        scene["pause_after_ms"] = max(0, min(500, _safe_int(scene.get("pause_after_ms"), 0)))
        ...
        scene["emotional_tone"] = _clean(scene.get("emotional_tone")) if _clean(scene.get("emotional_tone")) in TONES else ("urgent" if i == 0 else "satisfied" if i == 6 else "curious")
        scene["visual_priority"] = _clean(scene.get("visual_priority")) or "primary"
        scene["transition"] = _clean(scene.get("transition")) if _clean(scene.get("transition")) in TRANSITIONS else ("hard_cut" if i in (0, 1, 5) else "match_cut")
        ...
        for j, visual in enumerate(visuals):
            ...
            visual["visual_focus"] = _clean(visual.get("visual_focus")) or topic
            visual["visual_action"] = _clean(visual.get("visual_action")) or "show the exact physical action described in the spoken line"
            visual["must_show"] = [_clean(x)[:100] for x in visual.get("must_show", []) if _clean(x)][:6] or [visual["visual_focus"], visual["visual_action"]]
            visual["must_not_show"] = [_clean(x)[:100] for x in visual.get("must_not_show", []) if _clean(x)][:8]
            visual["camera"] = _clean(visual.get("camera")) if _clean(visual.get("camera")) in CAMERAS else ("close_up" if j == 0 else "macro")
            visual["animation"] = _clean(visual.get("animation")) if _clean(visual.get("animation")) in ANIMATIONS else (["zoom_in", "pan_right", "highlight", "parallax"][i % 4] if j == 0 else ["pan_left", "zoom_out", "highlight", "rotate"][i % 4])
            visual["zoom_strength"] = _clean(visual.get("zoom_strength")) or ("strong" if i == 0 else "medium")
            visual["motion_intensity"] = _clean(visual.get("motion_intensity")) or ("high" if i in (0, 5) else "medium")
            visual["visual_complexity"] = _clean(visual.get("visual_complexity")) or "focused"
            visual["image_style"] = _clean(visual.get("image_style")) if _clean(visual.get("image_style")) in IMAGE_STYLES else ("macro_photography" if visual["camera"] in {"close_up", "macro"} else "cinematic_photograph")
            visual["lighting"] = _clean(visual.get("lighting")) or "natural believable lighting with realistic reflections and shadows"
    ...
    script["image_generation"] = {"seed": int(time.time()), "style_lock": "realistic cinematic photography, natural materials, physically plausible lighting, no illustration look"}
    return script
