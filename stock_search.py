...
VERIFY_CANDIDATES = 6
TIMEOUT = 25
USER_AGENT = "Mint-YT-Factory/StockSearch/16.0"
...
def _anchor_terms(spoken: str, focus: str, action: str, must: list[str]) -> list[str]:
    ...
    stop = {
        "the", "and", "with", "from", "that", "this", "into", "under", "over", "when",
        "your", "their", "same", "visible", "showing", "shows", "because", "like",
        "really", "actually", "tiny", "little", "hard", "white", "yellow", "single",
        "physical", "object", "thing", "surface", "state", "scene", "shot", "camera",
    }
...
def _story_context(script: dict, scene_no: int) -> list[str]:
    ...
    if person:
        ...
        if any(x in lower_person for x in ("wade", "jordan", "lebron", "kobe", "curry")):
            terms.append("basketball player")
        elif any(x in lower_person for x in ("bose", "einstein", "tesla", "curie", "newton", "ramanujan")):
            terms.append("scientist")
        else:
            terms.append("historical person")
    keyword_map = [
        (r"basketball|court|arena|game|team|nba", "basketball court"),
        (r"contract|signed|deal|salary|million", "sports contract"),
        (r"school|college|university", "college campus"),
        (r"coach|coached", "sports coach"),
        (r"injur|hurt|pain|recover", "athlete training"),
        (r"family|mother|father|parent", "family support"),
        (r"poor|broke|money|struggle|homeless", "struggling person"),
        (r"practice|train|workout", "athlete training"),
        (r"championship|trophy|win|victory", "sports trophy"),
        (r"draft", "basketball draft"),
        (r"manuscript|journal|diary|letter|paper|book|wrote|writing", "old manuscript"),
        (r"research|theory|equation|discovery|experiment|scientific", "researcher writing"),
        (r"laboratory|lab", "research laboratory"),
        (r"lecture|professor|student", "university lecture"),
        (r"war|soldier|battle|army", "historical soldier"),
        (r"invention|machine|workshop", "inventor workshop"),
    ]
    for pattern, phrase in keyword_map:
        if re.search(pattern, text, re.I) and phrase not in terms:
            terms.append(phrase)
    if not terms:
        terms.append("historical person")
    return terms[:5]
...
def direct(scene_no: int, shot_no: int, scene: dict, visual: dict, failed_queries=None, round_no=1, story_script: dict | None = None):
    ...
    is_story = isinstance(story_script, dict) and bool(
        story_script.get("story_person") or story_script.get("interactive_pillar") or story_script.get("story_visual_mode")
    )
...
Return ONLY JSON with search_ladder, casting_brief, must_match and avoid.'''
    ...
    if is_story:
        ...
        safe: list[dict] = []
        for base in context:
            variants = [base, f"{base} action", f"{base} close up", f"{base} working", f"{base} indoors"]
            for query in variants:
                query = clean(query.lower(), 80)
                if query in existing or not 1 <= len(query.split()) <= 7:
                    continue
                if set(query.split()) & BAD_QUERY_WORDS:
                    continue
                safe.append({"query": query, "strategy": "story-deterministic"})
                existing.add(query)
        ladder = safe + ladder
    else:
        subject = " ".join(anchors[:2])
        variants = [
            subject, f"{subject} close up", f"{subject} being used", f"{subject} action",
            f"{subject} outdoors", f"{subject} indoors", f"{subject} hands",
        ] if subject else []
        for query in variants:
            query = clean(query.lower(), 80)
            if query and query not in existing and 1 <= len(query.split()) <= 7 and not (set(query.split()) & BAD_QUERY_WORDS):
                ladder.append({"query": query, "strategy": "deterministic"})
                existing.add(query)
    ...
    if len(ladder) < 4:
        ...
        for query in (base, f"{base} close up", f"{base} action", f"{base} outdoors", f"{base} hands"):
            query = clean(query.lower(), 80)
            if query and query not in existing and len(query.split()) <= 7:
                ladder.append({"query": query, "strategy": "local-fallback"})
                existing.add(query)
...
def build_plan(script):
    ...
    for si, scene in enumerate(scenes, 1):
        ...
        for vi, visual in enumerate(visuals, 1):
            directed = direct(si, vi, scene, visual, story_script=script if is_story else None)
...
def pexels(query, video):
    ...
    try:
        ...
        print(f"      ⚠️ Pexels {'VIDEO' if video else 'PHOTO'} request failed: {type(exc).__name__}")
        return []
...
def pixabay(query, video):
    ...
    try:
        ...
        print(f"      ⚠️ Pixabay {'VIDEO' if video else 'PHOTO'} request failed: {type(exc).__name__}")
        return []
        ...
        if video:
            ...
            for file in item.get("video_files") or []:
                ...
                if url:
                    ...
                    choices.append((portrait_penalty, abs(width * height - 720 * 1280), url))
...
def verify_actual(d, items, provider, video, query, strategy):
    ...
    for item in items[:VERIFY_CANDIDATES]:
        ...
        if not preview or not url or _asset_key(item, provider, video, url) in historical:
            continue
        try:
            parts.append(_image_part(_download_bytes(preview)))
...
def _deterministic_score(d, item, provider, video, query):
    hay = " ".join([
        query, str(item.get("alt", "")), str(item.get("description", "")),
        str(item.get("tags", "")), str(item.get("url", "")),
    ]).lower()
    anchors = d.get("anchor_terms", [])
    score = 0.0
    score += min(sum(1 for term in anchors if term.lower() in hay) * 1.5, 5.0)
    score += min(sum(1 for word in re.findall(r"[a-z0-9]+", query.lower()) if len(word) > 2 and word in hay) * 0.5, 2.5)
...
def _select_without_vision(d, items, provider, video, query):
    ...
    ranked = [
        ...
        if _url(item, provider, video)
        and _asset_key(item, provider, video, _url(item, provider, video)) not in historical
    ]
        ...
        for entry in d.get("search_ladder", []):
            ...
            for provider2, video2 in (("Pexels", True), ("Pixabay", True), ("Pexels", False), ("Pixabay", False)):
                ...
                for candidate in sorted(items, key=lambda x: _deterministic_score(d, x, provider2, video2, query2), reverse=True):
                    score = _deterministic_score(d, candidate, provider2, video2, query2)
                    if score < 2.5:
                        break
                    candidate_url = _url(candidate, provider2, video2)
                    candidate_key = _asset_key(candidate, provider2, video2, candidate_url) if candidate_url else ""
                    if not candidate_url or candidate_url in used_urls or candidate_url in failed_urls or candidate_url in queued or candidate_key in _historical_asset_keys():
                        continue
                    queue.append((candidate, provider2, video2, query2))
...
def generate_media(script, output_dir, config, gim=None):
    ...
    if is_story:
        print("🛡️ Story stock routing: scene-specific concrete queries first; Publish topic lock bypassed")
    ...
    for scene_no, shots in enumerate(plan, 1):
        for shot_no, directed in enumerate(shots, 1):
                ...
                for provider, video in (("Pexels", True), ("Pixabay", True), ("Pexels", False), ("Pixabay", False)):
                    ...
                    if vision_available:
                        try:
                            ...
                            if item:
                                print("      👁️ Vision verified actual thumbnail")
                        except Exception as exc:
                            vision_failures += 1
                    ...
                    if item:
                        ...
                        if url and url not in used and key not in _historical_asset_keys():
                            selected, selected_provider, selected_video, selected_query = item, provider, video, query
                            break
                if selected:
                    break
            ...
            if not selected:
                raise RuntimeError(
                    f"No new visually relevant stock asset found for Scene {scene_no} Shot {shot_no}. "
            ...
            recovered = _download_with_candidate_recovery(
                directed,
                (selected, selected_provider, selected_video, selected_query),
                path,
                used,
            )
            if not recovered:
                raise RuntimeError(f"No downloadable visually relevant NEW stock asset found for Scene {scene_no} Shot {shot_no} after candidate recovery.")
            selected, selected_provider, selected_video, selected_query, url = recovered
            used.add(url)
            _record_media_asset(selected, selected_provider, selected_video, url, scene_no, shot_no, selected_query)
            groups.append({
                "scene": scene_no,
                "shot": shot_no,
                "path": path,
