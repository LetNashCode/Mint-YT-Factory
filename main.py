...
def run(dry_run=False):
    ...
    print("="*80); print("🧠 LOADED CHANNEL LEARNING PLAYBOOK"); print("="*80); print(learning_context[:3500])
    print("="*80); print("✍️ GENERATING ENTERTAINING STORY WITH LEARNED PATTERNS"); print("="*80)
    script=generate_script(topic,config,None,extra_feedback=learning_context); script, next_topic=lock_next_topic(script,topic)
    workdir=os.path.join("output",str(int(time.time()))); os.makedirs(workdir,exist_ok=True); save_json(script,os.path.join(workdir,"script.json")); write_continuation_manifest(topic,next_topic,"locked",workdir)
    ...
    print("="*80); print("🎵 SELECTING MUSIC"); print("="*80); music=download_music(script,os.path.join(workdir,"music")); sfx=[]
    final_video=os.path.join(workdir,"final.mp4"); print("="*80); print("🎬 ASSEMBLING SHORT"); print("="*80); assemble_video(script,audio,visuals,music,sfx,config,final_video)
    ...
    title,description=build_youtube_metadata(script); print("="*80); print("🚀 UPLOADING SHORT"); print("="*80); upload_result=upload_video(final_video,title,description,config); print(f"✅ Upload completed: {upload_result}")
    write_continuation_manifest(topic,next_topic,"published",workdir)
    try:
        from youtube_analytics import record_upload
        record_upload(upload_result,topic,title,workdir,production_metadata={"topic_category":script.get("topic_category",topic),"hook_type":script.get("hook_type",script.get("scene_plan",[{}])[0].get("retention_purpose","")),"story_structure":"7-scene curiosity story","visual_style":script.get("visual_identity",{}).get("style",""),"music_type":script.get("music",{}).get("arc",""),"voice":script.get("voice_style",{}).get("tone","")})
        refresh_playbook()
