...
Missing words are reconstructed between anchors using the measured audio
interval, while caption timing is made continuous so words do not disappear
before the next spoken word starts.
"""
...
def _observed(result):
    ...
    for seg in result.get("segments", []) or []:
        ss = max(0.0, float(seg.get("start", 0)))
        se = max(ss + 0.05, float(seg.get("end", ss + 0.05)))
        for item in seg.get("words") or []:
            w = str(item.get("word", "")).strip()
            if not w:
                continue
            try:
                st = max(ss, float(item.get("start", ss)))
                en = min(se, max(st + 0.04, float(item.get("end", st + 0.04))))
...
def _alignment(expected, observed):
    """Find monotonic exact-word anchors without allowing weak substitutions.

    Whisper can insert, omit, or hallucinate words. The previous alignment
    treated a mismatch almost like a normal match, which could create anchors
    at the wrong point in the narration and then drag every reconstructed
    caption around that anchor. Exact matches now receive a much stronger
    score and mismatches are never emitted as anchors.
    """
    ...
    for i in range(n):
        for j in range(m):
            same = _clean(expected[i]) == _clean(observed[j]["word"])
    ...
    i, j = n, m
    while i and j:
        a = _clean(expected[i - 1])
        b = _clean(observed[j - 1]["word"])
...
def _word_weight(w):
    ...
    return max(1.0, letters * 0.78 + (0.55 if str(w).endswith((".", "!", "?")) else 0))
...
def _fill_gap(expected, left, right, left_time, right_time, out):
    ...
    weights = [_word_weight(expected[i]) for i in indexes]
    total = sum(weights) or 1.0
    cursor = left_time
    available = max(0.025, right_time - left_time)
    for i, weight in zip(indexes, weights):
        span = max(0.025, available * weight / total)
        out.append(
            {
                "expected_index": i,
                "word": expected[i],
                "start": cursor,
                "end": min(right_time, cursor + span),
            }
        )
        cursor += span
...
def _reconstruct(expected, anchors, duration):
    ...
    if not anchors:
        out = []
        _fill_gap(expected, 0, len(expected), 0.0, duration, out)
        return _finalize(out, duration)

    anchors = sorted(anchors, key=lambda x: x["expected_index"])
    out = []
    first = anchors[0]
    _fill_gap(expected, 0, first["expected_index"], 0.0, first["start"], out)
...
def _finalize(words, duration):
    ...
    for item in sorted(words, key=lambda x: x.get("expected_index", 0)):
        ...
        end = min(duration, max(start + 0.025, float(item["end"])))
        if start >= duration:
            start = max(0.0, duration - 0.025)
            end = duration
        out.append(
            {
                "word": str(item["word"]).strip(),
                "start": start,
                "end": max(start + 0.025, end),
            }
        )
        previous = out[-1]["end"]
    return out
...
def transcribe(audio_path):
    ...
    for name, strong in ((WHISPER_MODEL_NAME, True), (WHISPER_RETRY_MODEL_NAME, False)):
        try:
            observed = _observed(_transcribe(_get_model(name), audio_path, strong))
            anchors, cov = _alignment(expected, observed)
            if cov > coverage:
                best, coverage = anchors, cov
            if cov >= 0.90:
                break
        except Exception as error:
            print(f"⚠️ Whisper {name} failed: {type(error).__name__}: {error}")
    ...
    print(
        f"✅ TTS-locked captions: {len(timeline)} exact script words; "
        f"Whisper anchor coverage {coverage:.0%}; final audio {duration:.2f}s"
    )
    return timeline
