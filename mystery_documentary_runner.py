...
def main() -> None:
    ...
    candidates = [
        ...
        and (item.get("screening") or {}).get("eligible") is True
        and str(item.get("id")) not in used
    ]
    requested = os.getenv("MYSTERY_FOOTAGE_ITEM_ID", "").strip()
    if requested:
        candidates = [item for item in candidates if str(item.get("id")) == requested]
    if not candidates:
    ...
    runpy.run_module("mystery_documentary", run_name="__main__")
    ...
    history["used_item_ids"] = sorted(used | {item_id})
    history["entries"] = [*history.get("entries", []), entry]
