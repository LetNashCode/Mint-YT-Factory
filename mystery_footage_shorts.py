...
def validate_case(item: dict) -> None:
    missing = [field for field in REQUIRED_CASE_FIELDS if not item.get(field)]
    if missing:
        raise RuntimeError(
            f"Catalog case {item.get('id', '<unknown>')} is missing required fields: {', '.join(missing)}"
        )
    ...
    if not isinstance(item.get("verified_facts"), list) or not item["verified_facts"]:
        raise RuntimeError(f"Catalog case {item['id']} must contain a non-empty verified_facts list.")
    if not isinstance(item.get("theories_or_open_questions"), list):
        raise RuntimeError(f"Catalog case {item['id']} must contain a theories_or_open_questions list.")
...
def load_item():
    ...
    allow_unverified = os.getenv("MYSTERY_FOOTAGE_ALLOW_UNVERIFIED", "false").lower() in TRUE_VALUES
    eligible = []
    for item in items:
        if not item.get("video_url") or not item.get("source_url"):
            continue
        if "REPLACE_ME" in json.dumps(item):
            continue
        if not allow_unverified and item.get("rights_verified") is not True:
            continue
        validate_case(item)
    ...
    if eligible:
        return eligible[0]
    raise RuntimeError(
        "No eligible curated found-footage case is available. Add a case with verified facts, "
        "a matching footage URL, and rights metadata to mystery_footage_catalog.json. "
        "Generic automatic footage discovery is intentionally disabled."
    )
...
def generate_script(item, source: Path, work: Path):
    ...
    theories = "\n".join(f"- {theory}" for theory in item["theories_or_open_questions"]) or "- None supplied"
    prompt = f"""You are creating a respectful documentary-style found-footage mystery Short.
You are writing narration ONLY AFTER inspecting the supplied video frames.
The supplied footage must remain the central subject of the narration. The case metadata provides context, not permission to invent visuals.
Create an original, factual, tense 75-110 word English narration with a strong first-second hook, escalating curiosity, and a final question.
Do not claim that the footage shows anything not visible in the frames.
Do not invent dialogue, actions, dates, locations, identities, evidence, motives, or conclusions.
Use verified facts only as background context. Clearly label theories and unresolved questions as theories or questions.
Do not present paranormal claims as fact. Treat real victims and sensitive cases respectfully.
Return JSON only with video_title, narration, description_intro, tags.
Case title: {item['title']}
Case summary: {item['case_summary']}
Footage description: {item['footage_description']}
Verified facts:\n{verified_facts}
...
def render(source, audio, output, work):
    ...
    run([
        "ffmpeg", "-y", "-i", str(source), "-t", "55",
        "-vf", "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,fps=30",
        "-an", "-c:v", "libx264", "-preset", "veryfast", "-pix_fmt", "yuv420p", str(normalized),
    ])
    run([
        "ffmpeg", "-y", "-stream_loop", "-1", "-i", str(normalized), "-i", str(audio),
        "-map", "0:v:0", "-map", "1:a:0", "-t", "60", "-r", "30",
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="mystery-footage-") as temp_dir:
        ...
        print("Curated footage downloaded; inspecting frames before writing the narration.")
        script = generate_script(item, source, work)
        audio = work / "narration.mp3"
        synthesize_narration(
            clean(script["narration"]),
            {"voice": {"provider": "kokoro", "voice_name": os.getenv("MINT_KOKORO_VOICE", "af_heart"), "kokoro_lang": os.getenv("MINT_KOKORO_LANG", "a"), "speed": 1.0}},
            str(audio),
    ...
    description = (
        ...
        f"Case: {item['title']}\n"
        f"Source footage: {item['source_url']}\n"
        f"License: {item.get('license', 'Not specified')}\n"
        f"Attribution: {item.get('attribution', 'See source page for attribution requirements.')}\n\n"
    ...
    metadata = {
        ...
        "rights_verified": item.get("rights_verified"),
    ...
    if os.getenv("MYSTERY_FOOTAGE_AUTO_UPLOAD", "false").lower() in TRUE_VALUES:
        ...
        config = {
            "upload": {"privacy_status": os.getenv("MYSTERY_FOOTAGE_PRIVACY_STATUS", "private"), "category_id": "24"},
            "seo": {"hashtags": script["tags"]},
        }
        metadata["youtube_video_id"] = upload_video(
            str(output), clean(script["video_title"], 90), description, config,
            engagement_comment="What detail in this footage stands out to you?",
        )
        (OUTPUT_DIR / "metadata.json").write_text(json.dumps(metadata, indent=2, ensure_ascii=False), encoding="utf-8")
