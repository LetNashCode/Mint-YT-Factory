...
def patch_media_selection(media):
    ...
    def pick_video(results,required,actions):
        ...
        for item in results:
            page=item.get("url","")
            if not page or page in used: continue
        ...
        for score,item in sorted(ranked,key=lambda x:x[0],reverse=True):
            ...
            if files:
                page=item.get("url",""); used.add(page)
                return {"video":max(files)[2],"page":page,"photographer":(item.get("user") or {}).get("name",""),"score":score}
        return None
    ...
    def pick_photo(results,required,actions):
        ...
        for item in results:
            page=item.get("url","")
            if not page or page in used: continue
            w=int(item.get("width") or 0); h=int(item.get("height") or 0); src=item.get("src") or {}
            link=src.get("portrait") or src.get("large2x") or src.get("large") or src.get("original")
            if not link or not w or not h: continue
            score=(4 if h>=w else 0)+(2 if h/w>=1.15 else 0)
            ranked.append((score,item,link))
        for score,item,link in sorted(ranked,key=lambda x:x[0],reverse=True):
            if score<4: continue
            page=item.get("url",""); used.add(page)
            return {"photo":link,"page":page,"photographer":item.get("photographer","") or "","score":score}
        return None
    ...
    def generate_media(script,output_dir,config,gim):
        ...
        print(f"🎞️ Pexels unique assets used: {len(used)}")
        return groups
