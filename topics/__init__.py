...
# Deterministic emergency pool. These are only used when Gemini gets stuck;
# every candidate still passes the same everyday + duplicate + retired-topic gates.
_FALLBACK_TOPICS=(
...
def _deterministic_fallback(used):
    # Rotate the pool deterministically so repeated runs do not keep selecting its first item.
...
def _generate_topic(used):
    ...
    for attempt in range(1,11):
        try:
            ...
            if not _candidate_is_new(candidate, used):
                print("⚠️ Rejected: invalid, duplicate, or near-duplicate topic."); continue
            return candidate
...
def save_next_short(next_short):
    ...
    if existing_pending and not pending_duplicate:raise RuntimeError(f"Refusing to replace an existing pending continuation topic: {existing_pending[0]}")
    if pending_duplicate:return existing_pending[0]
