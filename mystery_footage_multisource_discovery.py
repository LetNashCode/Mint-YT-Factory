...
QUERIES = [
    "ufo video",
    "unexplained footage",
    "strange security camera",
    "unusual surveillance video",
    "paranormal footage",
    "mysterious recording",
    "unexplained phenomenon video",
    "odd historical film",
]
...
def choose_archive_video(identifier: str, files: list[dict[str, Any]]) -> dict[str, str] | None:
    ...
    for item in files:
        ...
        # Prefer MP4/WebM, then smaller playable files.
    ...
    for query in QUERIES:
        ...
        for doc in result.get("response", {}).get("docs", []):
            ...
            try:
                ...
                print(f"Archive candidate found: {identifier}")
                if len(found) >= limit:
                    return found
...
def discover_wikimedia(limit: int) -> list[dict[str, Any]]:
    ...
    for query in QUERIES:
        result = get_json(WIKIMEDIA_API_URL, {
            ...
            "gsrlimit": 25, "prop": "imageinfo", "iiprop": "url|mime|size|extmetadata",
        })
        for page in (result.get("query", {}).get("pages", {}) or {}).values():
            ...
            print(f"Wikimedia candidate found: {page_id}")
            if len(found) >= limit:
                return found
    return found
...
def save_candidates(candidates: list[dict[str, Any]]) -> None:
    ...
    for candidate in candidates:
        ...
        # Preserve a prior screening decision if the same candidate is rediscovered.
