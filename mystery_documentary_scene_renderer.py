...
def probe(path):
    ...
    d = json.loads(raw); v = next((x for x in d.get("streams", []) if x.get("codec_type") == "video"), {})
...
def download(item, target):
    ...
    if not url: raise RuntimeError("No video URL")
    r = requests.get(url, headers={"User-Agent": "Mozilla/5.0"}, timeout=180, stream=True); r.raise_for_status()
    if "text/html" in r.headers.get("content-type", "").lower(): raise RuntimeError("Video URL returned HTML")
...
def plan(item, source, work, duration):
    ...
    data = parse_json(result.text); raw_scenes = data.get("scene_plan")
    if not isinstance(raw_scenes, list) or not raw_scenes: raise RuntimeError("No scene plan returned")
    scenes=[]; cursor=0.0
    for s in raw_scenes:
        start=max(cursor, float(s.get("start", cursor))); end=min(duration, float(s.get("end", start)))
        if end <= start: continue
        mode=str(s.get("audio_mode", "original")).lower()
        if mode not in {"original","narration","pause","replay"}: mode="original"
        scenes.append({"start":round(start,3),"end":round(end,3),"audio_mode":mode,"narration":str(s.get("narration","")).strip(),"purpose":str(s.get("purpose","")),"evidence_label":str(s.get("evidence_label","observation"))}); cursor=end
    if cursor < duration-0.05: scenes.append({"start":round(cursor,3),"end":round(duration,3),"audio_mode":"original","narration":"","purpose":"Preserve uncovered footage","evidence_label":"observation"})
    data["scene_plan"]=scenes; return data

def render_scene(source, s, narration, out, work):
    start,end=s["start"],s["end"]; length=max(.1,end-start); mode=s["audio_mode"]
    video=work / (out.stem + ".video.mp4")
    vf="scale=1920:1080:force_original_aspect_ratio=increase,crop=1920:1080,setsar=1,fps=30"
    if mode == "pause": vf += ",select='eq(n,0)',tpad=stop_mode=clone:stop_duration=" + str(length)
...
def main():
    data=json.loads(CATALOG.read_text(encoding="utf-8")); requested=os.getenv("MYSTERY_FOOTAGE_ITEM_ID","").strip()
    ...
    if not candidates:
        if os.getenv("MYSTERY_FOOTAGE_SKIP_IF_NO_ELIGIBLE","false").lower() in {"1","true","yes"}: print("MYSTERY_DOCUMENTARY_SKIPPED=no eligible case"); return
        raise RuntimeError("No eligible case")
    item=candidates[0]; OUT.mkdir(parents=True,exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="mystery-scenes-") as td:
        work=Path(td); source=work/"source.mp4"; download(item,source); duration,w,h=probe(source)
        if duration < float(os.getenv("MYSTERY_FOOTAGE_MIN_SOURCE_SECONDS","90")): raise RuntimeError("Source footage is too short")
        if h>w: raise RuntimeError("Portrait source rejected")
        timeline=plan(item,source,work,duration); (OUT/"timeline.json").write_text(json.dumps(timeline,indent=2,ensure_ascii=False),encoding="utf-8")
        parts=[]
        for i,s in enumerate(timeline["scene_plan"]):
            audio=None
            if s["audio_mode"] in {"narration","pause","replay"} and s["narration"]:
                audio=work/f"narration-{i:03d}.mp3"; synthesize_narration(s["narration"],VOICE,str(audio),target_duration=max(1,s["end"]-s["start"]))
            part=work/f"part-{i:03d}.mp4"; render_scene(str(source),s,str(audio) if audio else None,part,work); parts.append(part)
        listing=work/"concat.txt"; listing.write_text("\n".join("file '"+p.as_posix()+"'" for p in parts)+"\n",encoding="utf-8")
        output=OUT/"mystery-documentary.mp4"; cmd(["ffmpeg","-y","-f","concat","-safe","0","-i",listing,"-c","copy",output])
    ...
    (OUT/"metadata.json").write_text(json.dumps(meta,indent=2,ensure_ascii=False),encoding="utf-8"); print(f"MYSTERY_DOCUMENTARY_OUTPUT={output}")
