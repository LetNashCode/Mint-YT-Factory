"""YouTube performance collector + resilient self-learning data source."""
...
def fetch_video_stats(video_ids):
    ...
    for start in range(0,len(video_ids),50):
        ...
        for attempt in range(3):
            try:
                response=yt.videos().list(part='snippet,statistics,contentDetails',id=','.join(batch),maxResults=50).execute(); break
...
def refresh_registry():
    ...
    for r in records:
        ...
        if not cur: continue
        r['title']=cur.get('title') or r.get('title',''); r['published_at']=cur.get('published_at') or r.get('published_at'); latest={'views':int(cur.get('views',0)),'likes':int(cur.get('likes',0)),'comments':int(cur.get('comments',0)),'engagement_rate':_engagement_rate(int(cur.get('views',0)),int(cur.get('likes',0)),int(cur.get('comments',0))),'checked_at':now}; latest.update(advanced.get(vid,{})); r['latest']=latest; snaps=r.get('snapshots',[]); snaps=snaps if isinstance(snaps,list) else []; snaps.append(latest); r['snapshots']=snaps[-60:]
    _write(REGISTRY_PATH,records); total_views=sum(int(x.get('latest',{}).get('views',0)) for x in records); total_likes=sum(int(x.get('latest',{}).get('likes',0)) for x in records); total_comments=sum(int(x.get('latest',{}).get('comments',0)) for x in records); count=len(records); live_ready=_has_live_metrics(records)
