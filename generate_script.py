...
7 scenes × exactly 3 visuals = exactly 21 images per Short.
...
STANDARD_SCENE_DURATIONS = [3, 5, 7, 7, 8, 8, 7]
...
def build_system_prompt(scene_count=7, target_seconds=45):
    return f"""
...
- Exactly {scene_count * 3} visuals total.
- Exactly 45 seconds.
...
Never say "Did you know", "in this video", "let's explore",
or "today we're going to".
...
scientifically. Never claim supernatural explanations are proven.

SAFETY:
No medical diagnosis/treatment, dangerous instructions, political or
religious persuasion, dangerous challenges, violence, gore, or
conspiracy theories presented as fact.

VISUAL STYLE:
Premium educational documentary.
Realistic scientific visualization, cinematic 3D reconstruction,
macro photography, realistic environments, accurate anatomy,
cinematic lighting, strong depth, dramatic scale, clear subjects.
Avoid generic AI art, fantasy, cartoons, random objects, excessive glow,
abstract blobs, text, fake labels, logos and watermarks.
...
The three shots must follow the narration in logical order and feel
like consecutive documentary shots from the same production.
...
Do NOT mention:
YouTube, narration, audio, subtitles, viewers, AI,
image generation, prompt quality.

No text, labels, captions, logos or watermarks.
...
def build_response_schema(scene_count=7):
    ...
    scene_schema = {
        ...
        "properties": {
            ...
            "retention_purpose": _enum(VALID_RETENTION_PURPOSE),
            ...
            "subtitle_style": _enum(VALID_SUBTITLE_STYLE),
            "emphasis_word": {"type": "string"},
            "duration": {"type": "integer"},
            "pause_after_ms": {"type": "integer"},
            "emotional_tone": _enum(VALID_EMOTIONAL_TONE),
            "visual_priority": _enum(VALID_VISUAL_PRIORITY),
            "transition": _enum(VALID_TRANSITION),
            "sfx_cue": {
                "type": "object",
                "properties": {
                    "term": {"type": "string"},
            ...
            "music_cue": _enum(VALID_MUSIC_CUE),
            "confidence": _enum(VALID_CONFIDENCE),
            "visuals": {
                "type": "array",
                "items": visual_schema,
            },
    ...
    return {
        ...
        "properties": {
            ...
            "voice_style": {
                ...
                "properties": {
                    "tone": {"type": "string"},
                    "pace": _enum(["slow", "medium", "fast"]),
                    "pitch": _enum(["low", "medium", "high"]),
                },
...
def _build_style_lock(visual_identity):
    style = str(
        visual_identity.get("style", "")
    ).strip()

    palette = str(
        visual_identity.get("palette", "")
    ).strip()

    mood = str(
        visual_identity.get("mood_arc", "")
    ).strip()

    parts = [
        value for value in (style, palette, mood)
        if value
    ]
    ...
    return (
        "Consistent premium educational documentary visual identity: "
        + ", ".join(parts)
        + "."
    )
...
def _enhance_image_prompt(
    ...
    """
    Keep the semantic prompt short.
    Add only the technical generation constraints needed by Pollinations.
    """
...
def _allocate_visual_durations(
    ...
    Durations always add up exactly to the scene duration.
    """
...
def _add_scene_visual_compatibility(
    ...
    """
    Preserve compatibility with existing generate_images.py.
    """
...
def validate_script(
    ...
    if len(scenes) != expected_scene_count:
        raise RuntimeError(
            f"Expected exactly {expected_scene_count} scenes "
    ...
    for index, scene in enumerate(
        ...
        # HARD RULE: exactly 3 images per scene.
        if not isinstance(
            visuals,
            list,
        ) or len(visuals) != 3:
            ...
            raise RuntimeError(
                f"Scene {index} must contain exactly "
                f"3 visuals but Gemini returned {actual}."
            )
        ...
        if visual_sum != duration:
            raise RuntimeError(
                f"Scene {index} visual durations sum to "
                f"{visual_sum}s but scene duration is "
                f"{duration}s."
            )
    ...
    if total_visuals != 21:
        raise RuntimeError(
            f"Total visuals must be exactly 21 "
            f"but is {total_visuals}."
        )
...
def generate_script(
    ...
    # Force requested production format even if config is stale.
    scene_count = 7
    target_seconds = 45
    ...
    print("Scenes: 7")
    print("Visuals per scene: 3")
    print("TOTAL IMAGES: 21")
    print("Target: 45s")
    print(topic)
    print("=" * 80)
...
- each of the 3 visuals must represent a different part of
  the scene narration
