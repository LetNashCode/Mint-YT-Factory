...
def _title(number,pillar,person):
    labels={
        ...
        "one_decision":"One Decision Changed Everything",
        "before_they_were_famous":"Before They Were Famous",
...
def _validate_story_contract(script):
    ...
    if not any(word in narration.lower() for word in ("failed","rejected","lost","struggled","risk","obstacle","difficult","survived","turned","changed")):
        print("⚠️ Story contract: no explicit obstacle keyword found; allowing factual story to continue.")
    final=str(scenes[-1].get("narration",""))
    if "subscribe" not in final.lower() or "follow" not in final.lower():
...
def run():
    ...
    previous=get_pending_story()
    pillar,topic,person=get_next_topic()
    number=next_story_number()
    if previous and number<=int(previous.get("number",0)):
        raise RuntimeError(f"Invalid story sequence state: next #{number} must follow pending Story #{previous.get('number')}.")
    print(f"📖 STORY SHORT #{number} | {pillar} | {person}")

    facts_note=""
    if previous: print(f"🔓 Previous story: {previous.get('person','')} — continuing with a completely new subject.")
...
{facts_note}"""
    ...
    os.makedirs(workdir,exist_ok=True); save(script,os.path.join(workdir,"script.json"))
    ...
    if not isinstance(audio,(str,os.PathLike)) or not os.path.isfile(os.fspath(audio)):
        raise RuntimeError(f"Story narration file invalid: {audio!r}")
    ...
    title=_title(number,pillar,person)
    ...
    record_topic(topic,pillar,title,vid,workdir,person=person)
    save_pending_story(pillar,topic,person,number)
    ...
    record_analytics(vid,topic,pillar,title,workdir,person=person)
    print("📊 Story comparison:",json.dumps(build_comparison(),ensure_ascii=False))
