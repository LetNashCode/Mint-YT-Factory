...
def _validate(script, topic):
    ...
    for si,scene in enumerate(scenes,1):
        narration=_clean(scene.get("narration")); total_words+=len(_words(narration))
        if not narration: errors.append(f"Scene {si}: empty narration"); continue
        if _hits(narration,CTA_WORDS): errors.append(f"Scene {si}: CTA/marketing language inside narration")
        if si<7 and _hits(narration,FUTURE_MARKERS): errors.append(f"Scene {si}: future-topic transition leaked into story")
        ...
        for vi,visual in enumerate(visuals,1):
            focus=_clean(visual.get("visual_focus")); action=_clean(visual.get("visual_action")); prompt=_clean(visual.get("image_prompt")); spoken=_clean(visual.get("spoken_line")); combined=" ".join((focus,action,prompt)); label=f"Scene {si} Shot {vi}"
            if len(prompt.split())<8: errors.append(f"{label}: image prompt too vague")
            if len(prompt.split())>60: errors.append(f"{label}: image prompt too long")
            if not focus or not action: errors.append(f"{label}: missing concrete focus/action")
            abstract_visual=_hits(combined,ABSTRACT_BEATS)
            if abstract_visual: errors.append(f"{label}: non-literal visual wording: {', '.join(abstract_visual[:4])}")
            if _abstract_word_hits(combined): errors.append(f"{label}: abstract visual action")
            if _hits(combined,CTA_WORDS): errors.append(f"{label}: CTA text in visual contract")
            if not _physical_action(combined): errors.append(f"{label}: no searchable physical action/state")
            if spoken:
                narration_words=set(_words(narration)); spoken_words={w for w in _words(spoken) if len(w)>=4}; overlap=len(spoken_words & narration_words)/max(1,len(spoken_words))
                if overlap<0.35: errors.append(f"{label}: spoken_line does not map to narration")
            if vi==2 and previous_focus and focus.lower()==previous_focus.lower(): errors.append(f"{label}: Shot 2 duplicates Shot 1 focus")
            previous_focus=focus
    if total_words<MIN_STORY_WORDS or total_words>MAX_STORY_WORDS: errors.append(f"story word count {total_words} outside {MIN_STORY_WORDS}–{MAX_STORY_WORDS}")
    return errors

def patch_story_generation(main):
    original=main.generate_script
    if getattr(original,"_mint_final_story_gate",False): return
...
"""
            ...
            last_error=" | ".join(errors[:8]); print(f"🛡️ Final story/visual gate: FAIL (attempt {attempt}/{MAX_ATTEMPTS}) — {last_error}")
        raise RuntimeError(f"Final story/visual quality gate failed after {MAX_ATTEMPTS} attempts: {last_error}")
    generate_script._mint_final_story_gate=True; main.generate_script=generate_script
