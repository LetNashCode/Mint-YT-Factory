...
The renderer uses CRF-based H.264 encoding. Bitrate can be very low for
simple still-image timelines, so bitrate is treated as a sanity check rather
than a hard quality guarantee. Structural video checks remain mandatory.
"""
...
MINIMUM_ACCEPTED_BITRATE_RATIO = 0.70


def _probe(path: str) -> dict:
    command = ["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries", "stream=width,height,r_frame_rate,avg_frame_rate,bit_rate,codec_name,pix_fmt,color_space,color_transfer,color_primaries", "-of", "json", path]
...
def validate_final_video(path: str, expected_bitrate_mbps: float = EXPECTED_BITRATE_MBPS) -> dict:
    ...
    if bitrate <= 0:
        ...
        errors.append(f"Video bitrate is below the accepted CRF-profile floor: {bitrate:.2f} Mbps")
    ...
    print("=" * 80)
    return {"ok": True, "path": path, "width": width, "height": height, "fps": fps, "codec": codec, "pixel_format": pixel_format, "bitrate_mbps": bitrate, "target_bitrate_mbps": expected_bitrate_mbps, "minimum_accepted_bitrate_mbps": minimum_bitrate}
