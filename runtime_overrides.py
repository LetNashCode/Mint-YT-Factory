...
def _sanitize_final_visuals(final, stale_text, current_topic):
    stale_tokens={w for w in re.findall(r"\b[a-z0-9]+\b",_clean(stale_text).lower()) if len(w)>=6}; visuals=final.get("visuals")
    ...
    for visual in visuals:
        ...
        if stale_tokens and stale_tokens.intersection(set(re.findall(r"\b[a-z0-9]+\b",combined.lower()))):
            visual["visual_focus"]=_clean(current_topic); visual["visual_action"]="show the final physical result clearly"; visual["must_show"]=[_clean(current_topic),"clear final physical state"]; visual["must_not_show"]=["future continuation topic","unrelated object","different mystery"]
...
def patch_continuation(main):
    def lock_next_topic(script,current_topic):
        ...
        used=[_clean(current_topic)]+[x for x in _read_used() if not str(x).startswith(_PENDING_PREFIX)]; canonical=""
        ...
        script.setdefault("next_short",{})["topic"]=canonical; scenes=script.get("scene_plan")
        if not isinstance(scenes,list) or len(scenes)!=7: raise RuntimeError("Script must contain exactly 7 scenes.")
        final=scenes[-1]; sentences=_split_sentences(final.get("narration","")); key=_topic_key(canonical); matches=[s for s in sentences if key in _topic_key(s)]
        if len(matches)!=1 or matches[0]!=sentences[-1]: raise RuntimeError("Gemini must put the locked next topic in Scene 7's final sentence.")
        ...
        final["subtitle_text"]=final["narration"]; final["pause_after_ms"]=150; final["emotional_tone"]="satisfied"; final["music_cue"]="fade_out"; final["caption_highlights"]=[{"word":w,"emphasis":"strong"} for w in _words(canonical)[:3]]; final["emphasis_word"]=_words(canonical)[0]
        ...
        print(f"🔒 Canonical next topic: {canonical}"); print(f"🗣️ GEMINI FINAL BRIDGE: {final['narration']}"); return script,canonical
