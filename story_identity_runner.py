...
def _compact_query(value: object) -> str:
    """Keep Commons queries short and remove prompt/instruction language."""
...
def _patch_archival_media_contract() -> None:
    ...
    # The underlying adapter historically reused an earlier asset when
    # Wikimedia could not provide a fresh one. That is unacceptable for a
    # two-shot-per-scene Story Short, so force a hard failure instead.
    ...
    def strict_generate_media(script, output_dir, config, gim=None):
        ...
        if len(groups) != 14:
            raise RuntimeError(
                f"Story visual quality gate failed: expected 14 assets, got {len(groups)}."
            )
        ...
        for item in groups:
            ...
            identity = source_url or asset_key
            if not identity:
